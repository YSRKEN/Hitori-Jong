				...
				<tbody>
					{unitCandidateData[0].map(unit => {
						return (<tr key={unit.id}>
							...
							<td>{UNIT_LIST2[unit.id].member.map(m => IDOL_LIST[m].name).join('・')}</td>
						</tr>);
					})
					}
				</tbody>
			...
			<table>
				...
				<tbody>
					{unitCandidateData[1].map(unit => {
						...
						const otherMember = unitInfo.member.filter(i => !unit.member.includes(i));
						return (<tr key={unit.id}>
							<td>{unitInfo.name}</td>
